...
public class ResponseBodyEncryptFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        String modifyMessage = FiledCrypt.encryptBody(jsonBody);
    ...
    private void jsonFieldPrint(String jsonBody){
        JsonNode node =null;
        try {
            node = FiledCrypt.getInstanceMapper().readTree(jsonBody);
        } catch (JsonProcessingException e) {
            e.printStackTrace();
             
        }

        JsonNode rootNode = node.at("/");

        Iterator<String> fieldNames = rootNode.fieldNames();
        while(fieldNames.hasNext()) {
            String fieldName = fieldNames.next();        
            JsonNode fieldNode = rootNode.get(fieldName);
            printAllJsonNodes(fieldName,fieldNode);
        }
