...
def DOCKER_CREDENTIALS_ID = "my-dockerhub"
def GIT_AUTH_CREDENTIALS_ID= "github-io203"


podTemplate(label: 'simple-api-job',
    containers: [
        containerTemplate(name: 'baseimg-build-tool', image: 'saturn203/baseimg-jdk17-skaffold-kustomize-git-docker:v1.0', command: 'sleep', args: '999999'),
    ]
) {
    node('simple-api-job') {
        stage('Checkout') {
        ...
        stage('gitOps'){
            ...
            container('baseimg-build-tool') {
                withCredentials([string(credentialsId: 'io203-github-token', variable: 'GITHUB_TOKEN')]) {
                    sh """
